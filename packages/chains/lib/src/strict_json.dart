...
/// Decodes JSON only when every object member has one unambiguous name.
///
/// Dart's standard decoder keeps the last value for duplicate keys. That is
/// unsafe for signed transaction envelopes because two implementations may
/// select different `transaction` or `txID` values. Escaped names are decoded
/// before comparison, so `transaction` and `trans\u0061ction` also collide.
Object? decodeJsonWithUniqueObjectMembers(String source, {int? maxChars}) {
  if (maxChars != null && (maxChars < 0 || source.length > maxChars)) {
    throw const FormatException('JSON record exceeds size limit');
  }
  final scanner = _UniqueJsonMemberScanner(source);
  if (scanner.scan()) {
...
class _UniqueJsonMemberScanner {
  _UniqueJsonMemberScanner(this.source);
