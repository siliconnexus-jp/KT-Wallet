...
extension AppThemeColors on AppTheme {
  ...
  Color get text => this == AppTheme.wallet ? WalletColors.text : SignerColors.text;
  Color get text2 =>
    ...
    return SizedBox(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Text('9:41', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: fg)),
            Row(children: [
              Icon(Icons.signal_cellular_alt, size: 16, color: fg),
              const SizedBox(width: 6),
              Icon(
                theme == AppTheme.signer ? Icons.airplanemode_active : Icons.wifi,
                size: 16,
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            ...
            if (navBar != null)
              Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: navBar!),
            Expanded(child: scrollable ? SingleChildScrollView(child: content) : content),
            if (bottom != null)
              Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 20), child: bottom!),
          ],
        ),
      ),
    );
...
class KtCard extends StatelessWidget {
  const KtCard({super.key, required this.child, this.theme = AppTheme.wallet, this.padding = const EdgeInsets.all(16)});
  final Widget child;
  final AppTheme theme;
  final EdgeInsets padding;
  @override
  Widget build(BuildContext context) => Container(
        padding: padding,
        decoration: BoxDecoration(color: theme.surface, borderRadius: BorderRadius.circular(KtDimens.radiusLg)),
        child: child,
      );
}
...
class KtAvatar extends StatelessWidget {
  const KtAvatar({super.key, required this.color, required this.initial, this.size = 40});
  final Color color;
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(initial, style: TextStyle(fontSize: size * 0.45, fontWeight: FontWeight.w600, color: Colors.white)),
      );
}

/// Segmented pill selector (fee tiers, filters, mnemonic length).
class KtSegmented extends StatelessWidget {
  const KtSegmented({super.key, required this.options, required this.selected, this.theme = AppTheme.wallet, this.onChanged});
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: Text(options[i],
                      style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: i == selected ? theme.bg : theme.text2)),
                ),
              ),
            ),
          ],
...
class _QrPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawFinder(int fx, int fy) {
      canvas.drawRect(Rect.fromLTWH(fx * cell, fy * cell, 7 * cell, 7 * cell), p);
      canvas.drawRect(Rect.fromLTWH((fx + 1) * cell, (fy + 1) * cell, 5 * cell, 5 * cell),
          Paint()..color = (ink == Colors.white ? const Color(0xFF0A0C0F) : Colors.white));
      canvas.drawRect(Rect.fromLTWH((fx + 2) * cell, (fy + 2) * cell, 3 * cell, 3 * cell), p);
    }
...
class KtTabBar extends StatelessWidget {
  const KtTabBar({super.key, this.selected = 0, this.items = const [
          ...
          child: Row(children: [
            for (var i = 0; i < items.length; i++)
              Expanded(
                child: Container(
                  decoration: BoxDecoration(
                    color: i == selected ? WalletColors.accent.withValues(alpha: 0.08) : null,
                    borderRadius: BorderRadius.circular(22),
                  ),
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(items[i].$2, size: 22, color: i == selected ? WalletColors.accent : WalletColors.text3),
                    const SizedBox(height: 2),
                    Text(items[i].$1,
                        style: TextStyle(fontSize: 10, fontWeight: i == selected ? FontWeight.w600 : FontWeight.w500, color: i == selected ? WalletColors.accent : WalletColors.text3)),
                  ]),
                ),
              ),
          ]),
