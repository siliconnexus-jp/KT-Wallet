...
func New(cfg Config) *Gateway {
	...
	g := &Gateway{
		...
		balanceCache: cache.NewShared(
			...
			balancesTTL,
			cfg.SharedCache,
			// v2 adds chain/network/address response identity. A distinct shared
			// namespace prevents a rolling 1.16.23 instance from decoding a
			// still-live pre-1.16.23 Redis row without those trust fields.
			"balances-v2",
