...
func (g *Gateway) Broadcast(ctx context.Context, params json.RawMessage) (any, *rpc.Error) {
	...
	if err := json.Unmarshal(params, &p); err != nil || len(params) == 0 {
		return nil, rpc.Errorf(rpc.CodeInvalidParams, `invalid params: expected {"chain", "payload"}`)
	}
	...
	case meta.EVM:
		...
		txHash, err = g.evm[p.Chain].SendRawTransaction(ctx, p.Payload)
	case p.Chain == "solana":
		...
		txHash, err = g.sol.SendTransaction(ctx, p.Payload)
	default: // tron
		...
		txHash, err = g.tron.Broadcast(ctx, []byte(trimmed))
	}
	if err != nil {
