...
func (s *Solana) GetBalance(ctx context.Context, address string) (*big.Int, error) {
	if !isValidSolanaPublicKey(address) {
		return nil, &Unavailable{Upstream: "solana", Message: "invalid account identity"}
	}
	raw, err := s.pool.Call(ctx, "getBalance", []any{address})
	if err != nil {
		return nil, err
	}
	v, err := decodeSolanaNativeBalance(raw)
	if err != nil {
...
func (s *Solana) GetTokenBalance(ctx context.Context, address, mint string) (*big.Int, error) {
	if !isValidSolanaPublicKey(address) || !isValidSolanaPublicKey(mint) {
		return nil, &Unavailable{Upstream: "solana", Message: "invalid token query identity"}
	}
	...
	total, err := decodeSolanaTokenBalance(raw, address, mint)
	if err != nil {
