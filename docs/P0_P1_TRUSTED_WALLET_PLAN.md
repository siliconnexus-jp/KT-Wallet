...
- [x] 2026-08-04 继续审阅 Gateway 其余公开参数边界，确认余额、Portfolio、价格、
  ...
  `Contract` 等大小写别名会被当作规范字段，重复 `limit` 及嵌套重复 Token contract
  会由后值覆盖。覆盖全部 12 个入口的 13 项确定性负例先全部失败：旧实现继续访问
  上游、返回成功或误报 unsupported。现全部迁移到共享递归精确 schema 解码器；加上
...
- [ ] 恶意/垃圾 Token 和可疑合约风险数据源：
  ...
  - [x] 2026-08-04 同类审计延伸到更高优先级的运营风险表：旧 loader 同样会忽略未知
    ...
    解码；未知字段和重复 key 红测先失败后通过。核心构造器额外持久化风险表有效性，任何
    无效程序化配置使 `kt_checkTokenRisk` 固定返回 operator_registry unavailable，绝不
  ...
  - [x] 2026-08-04 再次复核共享严格 JSON 实现发现 Go `encoding/json` 会忽略字段名
    ...
    同意。以上三类负例均在旧实现中稳定放行；修复后另加入嵌套 metric 的
    `SuccessCount` 别名回归，覆盖递归 schema 路径。
    现共享解码器先按目标 DTO 的精确 JSON tag 递归验证根对象、数组元素和嵌套结构，再做
...
- [ ] 隐私友好的 Crash/ANR、启动耗时、RPC 延迟与交易阶段指标。
  ...
  - [x] 匿名上传确认与余额、价格、历史、广播等生产 JSON-RPC 共用响应身份闭合：错
    ...
    JSON-RPC 所有权门禁。
  - [x] 2026-08-04 严审修复客户端聚合成功率误报：旧实现只要八链余额/历史任意一条
    返回 ok，就把整轮 `market.refresh` / `history.refresh` 记为成功，即使其他链明确
    error；价格服务失败后显示缓存时也被记为成功。现在请求范围内 missing/loading/error
    均使指标失败，明确 unsupported 保持中性但至少需要一个 live ok；可定价网络还必须
    获得每个启用原生币的正有限实时价格。UI 仍独立展示可用链和最后可信缓存，不因指标
    收紧制造整页失败。价格缓存回退、部分原生余额失败、部分 Token 余额失败、部分索引器
...
同日继续闭合移动端市场报价信任边界。旧 Gateway Client 会静默跳过畸形报价并展示剩余
币种，也会接受请求外币种、未知字段、缺失法币汇率、无缓存时间或陈旧缓存；直连
CoinGecko 路径还会接受重复 JSON 成员、部分币种和跨 quote 不一致汇率，并用残缺结果
覆盖最后可信缓存。失败优先测试先稳定复现四类错误展示，再修复为：Gateway 请求币种
规范化且禁止重复，响应只允许请求集合的子集、精确 quote/result/fiat schema、合理金融值
及 15 分钟新鲜度；直连路径必须返回完整请求 ID 集合、官方七字段、唯一 JSON 成员、正
有限价格、受限涨跌、每条新鲜时间，并要求 CNY/JPY 隐含汇率在 0.5% 内一致。任一坏行、
附加成员、缺失币种、陈旧时间或汇率污染会整体拒绝且不更新 last-good。定向 103/103、
KT Wallet 1618/1618（另有 11 项显式线上/设备测试默认跳过）、KT Cold Signer 570/570、
静态分析 0、远程安全边界、源码门禁 12/12 与完整依赖/OSV 门禁 13/13 通过。生产 Gateway
使用真实严格 App 客户端只读验收 6/6，其中完整 19 个已知币种、法币汇率与缓存时间均
通过新边界。本轮没有 Gateway 服务端源码变化，生产保持 1.16.25，无需重新部署；移动端
修复须随下一版签名 App 制品发布。没有视觉 UI 变化，因此不使用模拟器截图代替协议攻击
向量与真实 HTTPS 响应证据。

同日继续闭合移动端 Gateway 健康声明的网络路由权限。旧 App 会把缺少网络身份的旧版
健康响应默认解释为四条主网，也会静默接受重复、未知、非字符串网络 ID、附加字段和
畸形 upstream 摘要；运营方因此可能让自定义网络获得 Gateway 路由权限。失败优先测试
先证明附加字段仍返回 healthy、运营方声明的 custom network 实际接管余额请求，再修复为：
健康结果只接受精确 `ok/version/networks/upstreams`，版本必须为有界 `x.y.z` 形态，网络
列表必须为非空、唯一且完全属于当前 16 个内置网络；upstream 摘要键只能是同一已广告
集合的子集。旧版、空、重复、未知、附加或畸形声明一律失败闭合，自定义网络继续使用
用户配置的直连 RPC；构造时注入的缓存/测试声明也经过同一校验。定向 86/86、KT Wallet
1621/1621（另有 11 项显式线上/设备测试默认跳过）、KT Cold Signer 570/570、静态分析 0、
`check_deps` 与包含 69 个已审 Gateway 字段的远程安全边界通过。生产 Gateway 使用真实
...
同日继续复审授权时间证据。旧移动客户端只校验 `approvedAt >= 0`，被篡改的 Gateway 可返回
极远未来或超过 Dart `DateTime` 可表示范围的 Unix 秒；该值会进入授权对象并在列表格式化时
制造虚假未来时间或抛异常使页面崩溃。两个失败优先向量先证明旧实现接受当前时间两天后的
证据和超大秒数。修复后 App 独立执行与 Gateway 一致的边界：`0` 表示未知，其他值不得为负，
也不得超过本机当前 Unix 秒加 24 小时时钟容差；静态门禁锁定解析入口和容差计算。授权客户端
与 UI 定向 69/69；KT Wallet 1657/1657（另有 11 项显式线上/设备测试默认跳过）、
KT Cold Signer 570/570、共享 packages 438/438、静态分析 0、Gateway audit 与完整依赖/OSV
门禁 13/13 通过。生产 Gateway 使用公开零地址真实返回 `eth-mainnet/goplus/ok`、20 条授权，
...
同日继续复审钱包 PIN 与锁定状态的本地安全边界。旧实现只按 key 是否存在判断 PIN 已设置，
并用普通 `jsonDecode` 接受开放 schema、重复成员、任意 PBKDF2 轮数、任意盐/哈希长度和
攻击者可控的失败次数；损坏记录还可能让设置或转账认证 UI 抛异常。失败优先用例先证明未知
算法/字段、普通与 Unicode 转义重复 key、短盐、超大失败次数、缺失/超远锁定时间均会被
接受，且交易认证异常可逃逸。修复后 PIN 与 lockout 使用递归 duplicate-safe 闭合 schema，
强制安全记录不超过 4,096 字符、规范 Base64、16-byte salt、32-byte hash、PBKDF2 轮数不超过 1,000,000、失败次数不超过
64、单次锁定最长 24 小时、持久化时间最多在未来 7 天；`isSet` 必须完整解析记录。App Lock、
设置和转账入口均失败闭合并显示本地化安全存储错误，不会转入重新注册或绕过认证。PIN 定向
11/11、受影响 UI/认证定向 44/44、test_support 78/78、KT Wallet 1668/1668（另有 11 项
显式设备/线上测试默认跳过）、KT Cold Signer 570/570、共享 packages 438/438、静态分析 0、
Gateway audit 与完整依赖/OSV 门禁 13/13 通过。英文 390×844 Golden 已人工复核，证明损坏
状态只显示硬锁页面且不含钱包内容。Gateway 服务端无变化，无需重新部署；移动修复须随下一版
签名制品发布。

同日沿相同边界复审独立 KT Cold Signer。其旧 `PinLock` 同样使用普通 `jsonDecode`、开放
记录、presence-only `isSet`、无界 PBKDF2 轮数和失败次数位移；更关键的是，启动流程只读取
钱包 metadata，不验证 PIN/lockout，因此损坏 PIN 时仍可能开放 Face ID 签名路径。失败优先
测试先复现歧义记录、无界锁定、认证 sheet 异常逃逸及“损坏 PIN + 已启用生物识别仍进入
首页”。修复后 Signer 独立使用 duplicate-safe 闭合 schema、4,096 字符输入上限、规范
Base64、16/32-byte 盐与哈希、1,000,000 轮 PBKDF2 上限、64 次失败上限、24 小时锁定上限和
7 天未来时间边界；启动在原生地址派生及任何生物识别路由前验证 PIN 与 lockout，并仅在全部
读取成功后原子发布钱包状态。修改密码、签名 PIN sheet 和写入错误均显示本地化安全存储错误，
不会绕过或崩溃。PIN/启动/UI 定向 23/23、KT Cold Signer 576/576、KT Wallet 1668/1668、
...
同日继续复审 KT Wallet 的余额、价格与链上历史显示缓存。旧实现直接使用普通
`jsonDecode`，允许普通及 Unicode 转义同名字段以后值覆盖；顶层、金额和历史记录 schema
开放，JSON、Token map 与每链记录数没有输入上界。历史 `verified` 缺失或类型错误时还会
默认成 `true`，使损坏缓存短暂显示错误蓝勾。失败优先测试先稳定复现两类缓存接受歧义数据。
修复后市场缓存先限制 262,144 字符、最多 512 个 Token，历史缓存先限制 1,048,576 字符、
每链最多 100 条；两者统一递归拒绝嵌套重复 key，并按 v1/v2/v3 使用闭合顶层及记录 schema。
金额、链、时间、文本、价格、涨跌、法币与交易状态均有类型/范围限制；`verified` 必须显式为
bool，任一坏字段会丢弃整份显示缓存并继续实时刷新，不再部分吞错。旧 v1 市场/历史格式仍可
保守恢复。嵌套/Unicode 歧义、未知字段、负价格、未知链、超限文本/条数和缺失 verified 均
转绿；KT Wallet 1670/1670（另有 11 项显式设备/线上测试默认跳过）、KT Cold Signer
...
同日继续复审 AIRGAP-V1 的钱包身份与可见文本边界。旧协议只在外部 CBOR 解码后限制部分
UTF-8 长度，直接构造/编码可携带空 walletId、空白名称、换行、零宽字符与 Bidi 覆盖符；
KT Wallet 生成 SignRequest 时还会把超过 32 字符的 walletId 静默截断。walletId 是在线观察
钱包与离线签名权限的绑定标识，截断不是显示优化，而是把请求改写为另一个身份。失败优先
测试先证明五类协议对象可直接编码、恶意 CBOR 可被接受且 App 会继续生成截断请求。修复后
walletId 统一采用 1–32 字符规范 ASCII 身份语法；钱包名称、地址、派生路径、signer 与 txHash
统一要求非空、UTF-8 byte 有界并拒绝 C0/C1、零宽、Bidi 与其他方向控制字符。AccountExport
防御性冻结账户列表，构造、编码前对象与外部解码共用同一规则；App 精确保留 walletId，超限
立即失败，不再归一化或截断。仓库静态门禁锁定三类 payload 构造器、三条解码路径及在线请求
调用点。AIRGAP 共享包 70/70、Wallet 配对/签名 32/32、Signer 真实创建/签名/防重放 14/14、
test_support 88/88 与 `check_deps` 通过。该项没有读取私钥、没有广播或网络写入，也没有视觉
变化，因此不使用无关截图替代协议证据；最终 KT Wallet 1677/1677（另有 11 项显式设备/线上
测试默认跳过）、KT Cold Signer 583/583、共享 packages 457/457、静态分析 0 与公开源码门禁
12/12 全部通过。Gateway 无服务端变化，不部署后端。
