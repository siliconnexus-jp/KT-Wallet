...
class _SignerRecordsScreenState extends State<SignerRecordsScreen> {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context);
    final options = [l10n.filterAll, l10n.stateSigned, l10n.stateRejected, l10n.stateExpired];
    ...
    final rows = _filter == 0 ? all : all.where((r) => r.$5 == options[_filter]).toList();
    return KtScreen(
      theme: _t,
      gap: 16,
      navBar: KtNavBar(title: l10n.signRecords, theme: _t, onBack: () => Navigator.of(context).maybePop()),
      children: [
        KtSegmented(theme: _t, options: options, selected: _filter, onChanged: (i) => setState(() => _filter = i)),
...
class SignerWalletManageScreen extends StatefulWidget {
  ...
  @override
  State<SignerWalletManageScreen> createState() => _SignerWalletManageScreenState();
}
...
class _SignerWalletManageScreenState extends State<SignerWalletManageScreen> {
  ...
  Widget _row(IconData icon, String label, String sub, {bool danger = false, VoidCallback? onTap}) => GestureDetector(
  ...
  Future<void> _editName() async {
    ...
    // route.
    final controller = TextEditingController(text: _name ?? l10n.walletMainName);
    final result = await showDialog<String>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: SignerColors.surface,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text(l10n.editWalletName, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: SignerColors.text)),
        content: TextField(
          ...
          decoration: const InputDecoration(
            ...
            enabledBorder: UnderlineInputBorder(borderSide: BorderSide(color: SignerColors.border)),
            focusedBorder: UnderlineInputBorder(borderSide: BorderSide(color: SignerColors.ok)),
          ),
        ),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            child: Text(l10n.actionSave, style: const TextStyle(color: SignerColors.ok)),
          ),
        ],
      ),
    );
    if (result != null && result.isNotEmpty && mounted) setState(() => _name = result);
  }
    ...
    return KtScreen(
      ...
      children: [
        _card(Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(_name ?? l10n.walletMainName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: SignerColors.text)),
        ...
        _card(Column(children: [
          ...
          _row(Icons.checklist, l10n.mnemonicBackupCheck, l10n.mnemonicBackupCheckDesc, onTap: () => context.push('/mnemonic-show')),
          const SizedBox(height: 16),
          _row(Icons.qr_code, l10n.exportPublicAddress, '', onTap: () => context.push('/export')),
        ])),
        Container(
          ...
          child: _card(Column(children: [
            _row(Icons.delete_outline, l10n.deleteWallet, l10n.deleteWalletReqDesc, danger: true, onTap: () => context.push('/delete')),
            const SizedBox(height: 16),
            // No dedicated destroy-everything flow exists; the C21 delete flow
            // is the closest destructive path in the demo signer.
            _row(Icons.dangerous, l10n.destroyAllData, l10n.destroyAllDataDesc, danger: true, onTap: () => context.push('/delete')),
          ])),
        ),
      ],
    );
...
class SignerSecuritySettingsScreen extends StatefulWidget {
  ...
  @override
  State<SignerSecuritySettingsScreen> createState() => _SignerSecuritySettingsScreenState();
}

class _SignerSecuritySettingsScreenState extends State<SignerSecuritySettingsScreen> {
  /// In-memory verification policy (demo-stateless signer).
  ...
  Widget build(BuildContext context) {
    ...
    Widget toggleRow(IconData icon, String label, String sub, bool on, {Key? key, VoidCallback? onTap}) => GestureDetector(
          key: key,
          behavior: HitTestBehavior.opaque,
          onTap: onTap,
          child: Row(children: [
    ...
    return KtScreen(
      ...
      children: [
        ...
        _card(Column(children: [
          toggleRow(
            Icons.face, l10n.checkBiometric, l10n.biometricUsageDesc, _biometricOn,
            key: const ValueKey('toggle-biometric'),
            onTap: () => setState(() => _biometricOn = !_biometricOn),
          ),
          const SizedBox(height: 16),
          ...
          toggleRow(
            Icons.edit, l10n.verifyEverySign, l10n.verifyEverySignDesc, true,
            key: const ValueKey('toggle-verify-every-sign'),
            onTap: () => ScaffoldMessenger.of(context)
              ..clearSnackBars()
              ..showSnackBar(SnackBar(content: Text(l10n.verifyEverySignDesc))),
          ),
        ])),
    ...
    return KtScreen(
      ...
      bottom: Column(children: [
        ...
        GestureDetector(
          ...
          child: Text(l10n.actionCancel, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: SignerColors.text2)),
        ),
      ]),
      children: [
