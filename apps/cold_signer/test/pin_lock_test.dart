...
void main() {
  ...
  PinLock lock(InMemoryVaultStorage storage) => PinLock(
        storage,
        iterations: 500,
        random: Random(7),
        clock: () => now,
  ...
  test('hash roundtrip: enrolled PIN verifies, storage holds only a hash',
      () async {
    ...
    expect(base64Decode(record['salt']! as String).length, PinLock.saltLength);
  ...
  test('5 failures lock for 30s; further failures double the lockout',
      () async {
    final storage = InMemoryVaultStorage();
    final pin = lock(storage);
    await pin.setPin('123456');
  ...
  test('lockout persists across a restart (new PinLock, same storage)',
      () async {
    final storage = InMemoryVaultStorage();
    final pin = lock(storage);
    await pin.setPin('123456');
