...
void main() {
  ...
  testWidgets('full live loop: create → real PIN → sign → replay blocked',
    ...
    final wallet = controller(storage, records);
    ...
    expect(words, isNot(['ripple', 'canyon', 'script', 'harbor', 'velvet', 'noble', 'orbit', 'meadow', 'signal', 'pledge', 'quartz', 'ember']));
    ...
    expect(storage.values[SecureVault.mnemonicKey], words.join(' '));
    ...
    expect(row.walletId, demoWalletId);
  ...
  testWidgets('boot: existing wallet upgrades /welcome to the C5 home',
    ...
    addTearDown(tester.platformDispatcher.clearLocalesTestValue);
    final storage = InMemoryVaultStorage();
    await SecureVault(storage).storeWallet(
      mnemonic: ['abc'],
      metadata: const WalletMetadata(
          walletId: demoWalletId, name: '主钱包', createdAt: 1786000000),
    );

    await tester.pumpWidget(ColdSignerApp(
      walletController: controller(storage, InMemorySignRecordPersistence()),
      initialLocation: '/welcome',
    ));
    await tester.pumpAndSettle();
  ...
  testWidgets('delete wallet wipes the vault, PIN and records',
      (tester) async {
    tester.platformDispatcher.localesTestValue = <Locale>[const Locale('zh')];
    addTearDown(tester.platformDispatcher.clearLocalesTestValue);
    final storage = InMemoryVaultStorage();
    final records = InMemorySignRecordPersistence();
    await records.put(const SignatureRecord(
