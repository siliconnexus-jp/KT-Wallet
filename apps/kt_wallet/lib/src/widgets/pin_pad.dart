...
class PinDots extends StatelessWidget {
  const PinDots({super.key, required this.filled, this.colors = PinPadColors.wallet});
...
class PinPad extends StatelessWidget {
  const PinPad({super.key, required this.onKey, this.colors = PinPadColors.wallet});
            ...
            for (final k in row)
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                      color: k.isEmpty || k == 'del' ? null : colors.key, shape: BoxShape.circle),
                  child: k == 'del'
                      ? Icon(Icons.backspace_outlined, size: 20, color: colors.icon)
                      : Text(k,
                          style: TextStyle(
                              fontSize: 24, fontWeight: FontWeight.w500, color: colors.digit)),
                ),
              ),
          ]),
        ),
