...
/// actually move rows.
const _addresses = ChainAddresses(eth: '0xa', polygon: '0xa', tron: 'Ta', solana: 'a');
...
void main() {
  group('WalletController.reorder', () {
    ...
    test('reorders the list and persists sortOrder across a store reload', () async {
      final controller = WalletController(WalletManager(), store: WalletStore(db))
        ..add(_wallet('w1', 'A', 0))
        ..add(_wallet('w2', 'B', 1))
        ..add(_wallet('w3', 'C', 2));
  ...
  group('WalletManageScreen reorder mode', () {
    Widget app(WalletController controller) => MaterialApp(
          ...
          home: WalletScope(controller: controller, child: const WalletManageScreen()),
        );

    testWidgets('排序 toggles reorder mode; dragging row 0 below row 1 reorders and persists', (tester) async {
