...
void main() {
  test('the built-in registry carries the three canonical stablecoins', () {
  ...
  test('fetches ERC-20 (eth_call balanceOf) and TRC-20 balances (success)',
      () async {
  ...
  test('one failing endpoint degrades only that token', () async {
    final service = TokenBalanceService(
      ...
      restTransport:
          _FakeRest(onGet: (url) => throw TimeoutException('rest timeout')),
    );
  ...
  test('unactivated account / missing trc20 entry read as a real zero',
      () async {
    Future<Map<String, BalanceResult>> fetchWith(Object? tronBody) {
      final service = TokenBalanceService(
        jsonRpcTransport:
            _FakeJsonRpc((url, body) async => _rpcResult('0x0')),
  ...
  test('malformed responses surface as error status (never throw)', () async {
      ...
      restTransport: _FakeRest(onGet: (url) async {
        return {
          'data': [
            {
              'trc20': [
                {_usdtTron: 12345678} // non-string value: malformed, not zero
              ]
            }
          ]
        };
    ...
    final results = await service.fetchAll(_addresses);
    for (final token in builtinTokens) {
      expect(results[token.id]!.status, BalanceStatus.error,
          reason: token.id);
  ...
  test('endpoint resolver overrides the URLs the transports see', () async {
    ...
    expect(
        seenJsonUrls,
        containsAll(
            ['https://custom-eth.example', 'https://custom-polygon.example']));
