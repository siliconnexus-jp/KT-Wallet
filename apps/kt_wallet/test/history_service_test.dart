...
Map<String, Object?> _nativeTriggerItem({required String hash, required int ts}) => {
      'ret': [
        {'contractRet': 'SUCCESS'}
      ],
      'txID': hash,
...
void main() {
  ...
  test('TRON: parses TRC-20 + native fixtures, maps direction, merges newest first', () async {
    final service = _service(body: (request) {
      ...
      if (request.url.path.endsWith('/transactions/trc20')) {
        ...
        return {
          'data': [
            _trc20Item(hash: 'tx-out-usdt', from: _me, to: _other, ts: 3000),
            _trc20Item(
                hash: 'tx-in-usdt',
                from: _other,
                to: _me,
      ...
      return {
        'data': [
          _nativeTransferItem(hash: 'tx-out-trx', ownerHex: _meHex, ts: 2000),
          _nativeTransferItem(
    ...
    expect(result.records.map((r) => r.hash),
        ['tx-out-usdt', 'tx-out-trx', 'tx-in-usdt', 'tx-in-trx-failed']);
  ...
  test('TRON: unparseable token amount yields a null amountText, not a crash', () async {
    final service = _service(body: (request) {
      if (request.url.path.endsWith('/trc20')) {
        final item = _trc20Item(hash: 'tx-weird', from: _other, to: _me, ts: 100);
  ...
  test('TRON: empty data lists are a real (empty) history, not an error', () async {
    final service = _service(body: (_) => {'data': <Object?>[], 'success': true});
    final result = await service.fetch(Coin.tron, _me);
    expect(result.status, HistoryStatus.ok);
    expect(result.records, isEmpty);
  });

  test('Ethereum / Polygon / Solana are honestly unsupported (no keyless API)', () async {
    final service = _service(body: (_) => fail('no request expected'));
    for (final coin in [Coin.eth, Coin.polygon, Coin.solana]) {
      final result = await service.fetch(coin, '0xabc');
      expect(result.status, HistoryStatus.unsupported, reason: '$coin');
      expect(result.records, isEmpty);
    }
  });

  test('a 400 for an invalid (demo mock) address surfaces as error status', () async {
  ...
  test('one failing endpoint of the pair collapses to error (never partial-as-live)', () async {
    final service = _service(body: (request) {
      if (request.url.path.endsWith('/trc20')) {
        return {'data': <Object?>[], 'success': true};
      }
