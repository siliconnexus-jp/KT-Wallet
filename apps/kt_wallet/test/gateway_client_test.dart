...
void main() {
  group('GatewayClient request framing', () {
    test('JSON-RPC 2.0 envelope: jsonrpc/id/method/params, POST {url}/rpc',
        () async {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example/', client: recorder.client);
    ...
    test('health(): false on {ok:false}, error answers and transport failure',
        () async {
      final recorder = _Recorder()
        ..results = {
          'kt_health': {'ok': false}
      ...
      expect(
          await GatewayClient(
                  baseUrl: 'https://gw.example', client: recorder.client)
              .health(),
          isFalse);
      ...
      expect(
          await GatewayClient(baseUrl: 'https://gw.example', client: dead)
              .health(),
          isFalse);
    });

    test('timeout surfaces as an exception (10s default is configurable)',
        () async {
      ...
      await expectLater(
          client.getPrices(const ['ETH']), throwsA(isA<TimeoutException>()));
    });

    test('non-200 HTTP status throws (transport failure, not GatewayException)',
        () async {
      final client = GatewayClient(
          baseUrl: 'https://gw.example',
          client: MockClient((request) async => http.Response('oops', 502)));
      await expectLater(client.getPrices(const ['ETH']),
          throwsA(isA<http.ClientException>()));
    });
  });

  group('kt_getBalances', () {
    test('exact params and typed native + per-token rows (incl. errors)',
        () async {
      final recorder = _Recorder()
        ..results = {
          'kt_getBalances': {
      ...
      final balances = await client.getBalances(
        ...
        tokens: const [
          GatewayTokenQuery(
              ...
              decimals: 6,
              symbol: 'USDT'),
          GatewayTokenQuery(contract: '0xBadToken', decimals: 6, symbol: 'BAD'),
        ],
      );
    ...
    test('native-only call omits the tokens param; chain names match the enum',
        () async {
      final recorder = _Recorder()
        ..results = {
          'kt_getBalances': {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example', client: recorder.client);
      final balances =
          await client.getBalances(chain: Coin.tron, address: 'TTronAddr');
      expect(recorder.requests.single['params'],
          {'chain': 'tron', 'address': 'TTronAddr'});
      expect(balances.native.raw, BigInt.from(5000000));
      expect(balances.tokens, isEmpty);
      // The wire names of all four chains.
    ...
    test('malformed result (missing native) throws FormatException', () async {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example', client: recorder.client);
      await expectLater(client.getBalances(chain: Coin.eth, address: '0xA'),
          throwsA(isA<FormatException>()));
    });
  });

  group('kt_getPrices', () {
    test('exact params; unknown symbols omitted by the gateway stay absent',
        () async {
      final recorder = _Recorder()
        ..results = {
          'kt_getPrices': {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example', client: recorder.client);
      final prices = await client.getPrices(const ['ETH', 'POL', 'TRX', 'SOL']);
  ...
  group('kt_getChainParams', () {
    test('decimal nonce and three fee tiers parse into chains/rpc types',
        () async {
      final recorder = _Recorder()
        ..results = {
          'kt_getChainParams': {
            'nonce': '42',
            'fees': {
              'slow': {'maxPriorityFeePerGas': '1000000000', 'maxFeePerGas': '20000000000'},
              'standard': {'maxPriorityFeePerGas': '2000000000', 'maxFeePerGas': '30000000000'},
              'fast': {'maxPriorityFeePerGas': '3000000000', 'maxFeePerGas': '40000000000'},
            },
          }
        };
      final client =
  ...
  group('kt_getHistory', () {
    test('ok status with records; malformed rows are skipped', () async {
      ...
      final history = await client.getHistory(
          chain: Coin.eth, address: '0xEthAddr', limit: 20);

      expect(recorder.requests.single['params'],
          {'chain': 'eth', 'address': '0xEthAddr', 'limit': 20});
    ...
    test('status unsupported maps to the typed unsupported result', () async {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example', client: recorder.client);
      final history =
          await client.getHistory(chain: Coin.solana, address: 'SolAddr');
  ...
  group('kt_broadcast and error mapping', () {
    test('payload passthrough and txHash back', () async {
      ...
      final client =
          GatewayClient(baseUrl: 'https://gw.example', client: recorder.client);
      final hash =
          await client.broadcast(chain: Coin.eth, payload: '0x02ab01');
      expect(hash, '0xfeedbead');
      expect(recorder.requests.single['params'],
          {'chain': 'eth', 'payload': '0x02ab01'});
    });

    test('-32000 upstream_error surfaces the node message and upstream',
        () async {
      final recorder = _Recorder()
        ..errors = {
          'kt_broadcast': {
      ...
      await expectLater(
        ...
        throwsA(isA<GatewayException>()
            ...
            .having(
                (e) => e.upstreamMessage, 'upstreamMessage', 'nonce too low')),
      );
    });

    test('-32002 unsupported and -32001 rate_limited map onto the flags',
        () async {
      final recorder = _Recorder()
        ..errors = {
