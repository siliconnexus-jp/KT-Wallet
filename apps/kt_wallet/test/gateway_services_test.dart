...
class _FakeGateway {
  ...
  List<Map<String, Object?>> paramsOf(String method) =>
      [for (final (m, p) in calls) if (m == method) p];
}
...
void main() {
  group('BalanceService gateway mode', () {
    test('one kt_getBalances per chain, direct transports untouched',
        () async {
      final gateway = _FakeGateway(results: {
        'kt_getBalances': [
      ...
      expect({for (final p in params) p['chain']},
          {'eth', 'polygon', 'tron', 'solana'});
      expect(params.firstWhere((p) => p['chain'] == 'tron')['address'],
          'TTronAddr');
  ...
  group('TokenBalanceService gateway mode', () {
    test('one call per chain with its registry tokens; per-token errors map',
        () async {
      final gateway = _FakeGateway(results: {
        'kt_getBalances': {
      ...
      final params = gateway.paramsOf('kt_getBalances');
      expect({for (final p in params) p['chain']}, {'eth', 'polygon', 'tron'});
    ...
    test('gateway failure falls back to the direct path for that chain',
        () async {
      final direct = _FakeJsonRpc((url, body) async => _rpcResult(
          '0x00000000000000000000000000000000000000000000000000000000072f2740'));
      final rest = _FakeRest(onGet: (url) async => {
            'data': [
              {
                'trc20': [
  ...
  group('PriceService gateway mode', () {
    test('kt_getPrices first: symbols asserted, CoinGecko never contacted',
        () async {
      final gateway = _FakeGateway(results: {
        'kt_getPrices': {
          'prices': {
      ...
      final service = PriceService(
        client: MockClient((request) async =>
            fail('gateway mode must not contact CoinGecko (${request.url})')),
        gateway: () => gateway.client,
      );
  ...
  group('ChainParamsService gateway mode', () {
    test('kt_getChainParams first, direct transport untouched', () async {
      ...
      expect(gateway.paramsOf('kt_getChainParams').single,
          {'chain': 'eth', 'address': '0xFrom'});
    });

    test('gateway failure falls back to direct getNonce+estimateFees',
        () async {
      final direct = _FakeJsonRpc((url, body) async {
    ...
    test('both gateway and direct failing still throws (caller fallback)',
        () async {
      final service = ChainParamsService(
        jsonRpcTransport:
            _FakeJsonRpc((url, body) async => throw RpcException('down')),
        gateway: _deadGateway,
      );
      await expectLater(service.fetchEvmParams(Chain.ethereum, '0xFrom'),
  ...
  group('HistoryService gateway mode', () {
    ...
    test('gateway failure: TRON falls back direct, eth surfaces error',
        () async {
      var tronGridHits = 0;
      final service = HistoryService(
        client: MockClient((request) async {
      ...
      final tron =
          await service.fetch(Coin.tron, 'TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t');
    ...
    test('direct mode is unchanged: eth/polygon/solana unsupported, no '
      ...
      final service = HistoryService(
        client: MockClient(
            (request) async => http.Response(jsonEncode({'data': <Object?>[]}), 200)),
        gateway: () => null,
      );
      expect((await service.fetch(Coin.eth, '0xA')).status,
          HistoryStatus.unsupported);
      expect((await service.fetch(Coin.polygon, '0xA')).status,
          HistoryStatus.unsupported);
      expect((await service.fetch(Coin.solana, 'S')).status,
          HistoryStatus.unsupported);
      expect((await service.fetch(Coin.tron, 'TTronAddr')).status,
          HistoryStatus.ok);
    });
  });

  group('BroadcastService gateway mode', () {
    test('kt_broadcast per chain with the contract payload encodings',
        () async {
      final gateway = _FakeGateway(results: {
        'kt_broadcast': [
          {'txHash': '0xevm'},
      ...
      final tronJson = '{"raw_data":{"ref_block_bytes":"1234"},"signature":["aa"]}';
      final tron = await service.broadcast(
          Chain.tron, Uint8List.fromList(utf8.encode(tronJson)));
      expect(tron.txHash, 'tronTxid');

      final params = gateway.paramsOf('kt_broadcast');
      expect(params[0], {'chain': 'eth', 'payload': '0x02ab01'});
      expect(params[1],
          {'chain': 'solana', 'payload': base64Encode(solanaBytes)});
    ...
    test('-32000 node rejection: error with the node message, NO direct '
      ...
      final gateway = _FakeGateway(errors: {
        'kt_broadcast': {
          'code': -32000,
      ...
      final outcome = await service.broadcast(
          Chain.ethereum, Uint8List.fromList([0x02, 0x01]));
    ...
    test('gateway unreachable / unsupported: direct fallback posts the tx',
        () async {
      ...
      expect(direct.calls.single, (defaultEthRpcUrl, 'eth_sendRawTransaction'));
  ...
  group('prefsGatewayResolver (settings-driven mode switch)', () {
    ...
    test('after clearing the URL, a service takes the direct path again',
        () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = AppPrefsController();
      await prefs.load();
      ...
      final rest = _FakeRest(onGet: (url) async => {'data': <Object?>[]});
      final gateway = _FakeGateway(results: {
        'kt_getBalances': _native('0', 18, 'ETH'),
      });
      final service = BalanceService(
        jsonRpcTransport: direct,
        restTransport: rest,
