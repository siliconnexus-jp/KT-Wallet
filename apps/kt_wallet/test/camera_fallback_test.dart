...
void main() {
  testWidgets('unavailable camera renders the simulated viewfinder; tap simulates',
      (tester) async {
    var taps = 0;
    final scans = <String>[];
  ...
  testWidgets('KtCameraScreen falls back identically with a scan callback wired',
      (tester) async {
    var simulated = 0;
    await tester.pumpWidget(MaterialApp(
      home: KtCameraScreen(
  ...
  group('scannedAddressCandidate', () {
    test('accepts an address valid on any supported chain', () {
      ...
      expect(scannedAddressCandidate(' ${ScanAddressScreen.demoAddress} '),
          isNotNull, reason: 'whitespace from the QR content is trimmed');
    });
