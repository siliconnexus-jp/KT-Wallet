...
void main() {
  test('AccountExport round-trip: fragment → aggregate → decode equals source', () {
    final frames = demoAccountExportFrames();
    expect(frames.length, greaterThan(1), reason: 'demo chunk size must exercise aggregation');
    ...
    for (var i = 0; i < 4; i++) {
      ...
      expect(decoded.accounts[i].address, demoAccountExport.accounts[i].address);
      expect(decoded.accounts[i].path, demoAccountExport.accounts[i].path);
    }
  });

  testWidgets('scan → import-confirm shows decoded export → creates the paired watch wallet',
      (tester) async {
    tester.platformDispatcher.localesTestValue = <Locale>[const Locale('zh')];
    addTearDown(tester.platformDispatcher.clearLocalesTestValue);

    final controller = WalletController(WalletManager(initial: [
      HotWallet(
        id: 'daily',
        name: '日常钱包',
    ...
    await tester.pumpWidget(KtWalletApp(controller: controller, initialLocation: '/connect-cold'));
    ...
    expect(find.text('TcPa2Wc8…7L3kFa'), findsNothing); // sanity: truncation format checked below
